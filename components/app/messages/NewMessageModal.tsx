...
export function NewMessageModal({
    ...
    // Helper to get full name or email
    const getMemberDisplayName = (member: typeof members[number]) => {
        const { first_name, last_name, email } = member.user;
        return first_name || last_name ? `${first_name ?? ""} ${last_name ?? ""}`.trim() : email;
    };
    ...
    const handleAddTo = (userId: string) => setToUserIds([...toUserIds, userId]);
    const handleRemoveTo = (userId: string) =>
        setToUserIds(toUserIds.filter((id) => id !== userId));
    const handleAddCc = (userId: string) => setCcUserIds([...ccUserIds, userId]);
    ...
    const handleSubmit = async () => {
        ...
        if (!content.trim()) return setError("Message content is required");
        if (toUserIds.length === 0) return setError("At least one recipient (To) is required");
    ...
    const selectedToMembers = members.filter((m) => toUserIds.includes(m.user_id));
    const selectedCcMembers = members.filter((m) => ccUserIds.includes(m.user_id));
