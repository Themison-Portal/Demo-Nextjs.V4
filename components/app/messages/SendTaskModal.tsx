/**
 * Send Task Modal
 * Modal for sending a task as a message attachment
 */
...
export function SendTaskModal({
  ...
  const { user } = useAuth()
  const { teamMembers } = useTeamMembers(orgId, task.trial_id)
  const { createThread, isCreating } = useCreateThread(orgId)
  const { validate, isValidating } = useValidateTrialAccess(
    orgId,
    task.trial_id
  )
