-- ============================================================================
-- UPDATE HYDRATION FUNCTIONS TO POPULATE visit_activity_id
-- ============================================================================
-- Modifies hydrate_screening_visit() and hydrate_remaining_visits() to:
-- 1. Capture visit_activity.id after INSERT
-- 2. Use it to populate tasks.visit_activity_id (creating 1:1 relationship)
-- ============================================================================

-- ============================================================================
-- PART 1: Update hydrate_screening_visit
-- ============================================================================
...
DECLARE
  ...
  v_activity_record_id UUID; -- NEW: Store activity ID
...
BEGIN
  ...
  LOOP
    ...
    -- Create visit_activity and CAPTURE ID
    ...
    RETURNING id INTO v_activity_record_id; -- CAPTURE ID
    ...
    -- Create task with visit_activity_id
    INSERT INTO tasks (
      trial_id,
      patient_id,
      visit_id,
      visit_activity_id, -- NEW: Link to activity
      activity_type_id,
      title,
      status,
      priority,
    ...
    VALUES (
      ...
      p_patient_id,
      v_visit_record.id,
      v_activity_record_id, -- NEW: Use captured activity ID
      v_activity_id,
...
-- ============================================================================
-- PART 2: Update hydrate_remaining_visits
-- ============================================================================
...
COMMENT ON FUNCTION hydrate_screening_visit(UUID, UUID, DATE) IS
  'Creates ONLY the screening visit (order=1) and its tasks.
   ...
   NOW populates tasks.visit_activity_id for 1:1 relationship.
   Returns JSONB summary of created records.';

COMMENT ON FUNCTION hydrate_remaining_visits(UUID, UUID, DATE) IS
  'Creates remaining visits (order > 1, including Day 0) and their tasks.
   Called when patient is enrolled with baseline_date.
   NOW populates tasks.visit_activity_id for 1:1 relationship.
   Returns JSONB summary of created records.';
