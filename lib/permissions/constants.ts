...
export type OrgRole = 'superadmin' | 'admin' | 'editor' | 'reader' | 'staff';
...
const ORG_PERMISSIONS: Record<OrgRole, OrgPermissions> = {
    ...
    staff: {    canViewAllTrials: true,    canCreateTrial: true,    canInviteMembers: true,    canRemoveMembers: true,    canManageOrg: true,    canViewOrgSettings: true,  },  admin: {
        canViewAllTrials: true,
        canCreateTrial: true,
        canInviteMembers: true,
        canRemoveMembers: true,
...
export const ORG_ROLES: OrgRole[] = ['superadmin', 'admin', 'staff', 'editor', 'reader'];

/** Admin org roles that have full access */
export const ADMIN_ORG_ROLES: OrgRole[] = ['superadmin', 'admin', 'staff'];
